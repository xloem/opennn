...
/// Returns true if the neural network object has a principal components layer inside,
/// and false otherwise.
...
bool NeuralNetwork::has_principal_components_layer() const
{
    ...
    for(Index i = 0; i < layers_number; i++)
    {
        if(layers_pointers[i]->get_type() == Layer::Type::PrincipalComponents) return true;
    }
...
PrincipalComponentsLayer* NeuralNetwork::get_principal_components_layer_pointer() const
{
    ...
    for(Index i = 0; i < layers_number; i++)
    {
        if(layers_pointers[i]->get_type() == Layer::Type::PrincipalComponents)
        {
